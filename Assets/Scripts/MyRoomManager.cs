...
public class MyRoomManager : NetworkRoomManager
{
    ...
    public static new MyRoomManager singleton => (MyRoomManager)NetworkRoomManager.singleton;
    ...
    {
        Transform bottom = GameObject.Find("SpawnBottom")?.transform;
        Transform top = GameObject.Find("SpawnTop")?.transform;

        GameObject player;

        int playerIndex = roomPlayer.GetComponent<NetworkRoomPlayer>().index;  //keep track of players in server 

        if (playerIndex == 0)
        {
            player = Instantiate(player1Prefab, bottom.position, Quaternion.identity);
        ...
        {
            player = Instantiate(player2Prefab, top.position, Quaternion.Euler(0f, 0f, 180f));
            movement m = player.GetComponent<movement>();
            m.side = movement.Side.Top;
            return player;
        }

        /*if (playerIndex == 0 && bottom != null) { //for bottom player (player 1)

            return Instantiate(playerxPrefab, bottom.position, Quaternion.identity);
        }
        
        if (playerIndex == 1 && top != null) //for top player (player 2)
        {
            return Instantiate(playerxPrefab, top.position, Quaternion.Euler(0, 0, 180f));
        }
        return Instantiate(playerxPrefab);
        */
